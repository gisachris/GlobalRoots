...
import { SearchIcon, FilterIcon, MapPinIcon, ClockIcon, BriefcaseIcon, GlobeIcon, CheckIcon, ChevronDownIcon, TrendingUpIcon } from 'lucide-react';
...
export const YouthOpportunity = () => {
  ...
  // Mock opportunities data (excluding returnships)
  const opportunities = [{
    id: 1,
    type: 'mentor',
    name: 'David Mugisha',
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8 text-center">
          <div className="inline-flex items-center px-4 py-2 bg-[#B45309]/10 rounded-full border border-[#B45309]/20 mb-4">
            ...
            <span className="text-sm font-medium text-[#7C2D12] dark:text-gray-300">Youth Opportunities</span>
          </div>
          ...
          <p className="text-xl text-[#7C2D12] dark:text-gray-300 max-w-2xl mx-auto">
            Discover mentors, jobs, and micro-internships tailored to your skills and career goals
          </p>
        </div>
        ...
        {/* Tabs (without Returnships) */}
        <div className="mb-6 overflow-x-auto">
          <div className="flex space-x-2 pb-2">
            <button 
              className={`px-4 py-2 rounded-md whitespace-nowrap ${activeTab === 'all' ? 'bg-primary-600 text-white' : 'bg-white dark:bg-dark-800 text-dark-600 dark:text-primary-400'}`} 
              onClick={() => setActiveTab('all')}
            >
              All
            </button>
            <button 
              className={`px-4 py-2 rounded-md whitespace-nowrap ${activeTab === 'mentor' ? 'bg-primary-600 text-white' : 'bg-white dark:bg-dark-800 text-dark-600 dark:text-primary-400'}`} 
              onClick={() => setActiveTab('mentor')}
            >
              Mentors
            </button>
            <button 
              className={`px-4 py-2 rounded-md whitespace-nowrap ${activeTab === 'job' ? 'bg-primary-600 text-white' : 'bg-white dark:bg-dark-800 text-dark-600 dark:text-primary-400'}`} 
              onClick={() => setActiveTab('job')}
            >
              Jobs
            </button>
            <button 
              className={`px-4 py-2 rounded-md whitespace-nowrap ${activeTab === 'microinternship' ? 'bg-primary-600 text-white' : 'bg-white dark:bg-dark-800 text-dark-600 dark:text-primary-400'}`} 
              onClick={() => setActiveTab('microinternship')}
            >
              Micro-internships
            </button>
          </div>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {opportunity.skills.map((skill, idx) => (
                        <span key={idx} className="px-2 py-1 bg-primary-100 dark:bg-primary-900/50 text-primary-700 dark:text-primary-300 rounded-full text-xs">
                          {skill}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center">
                      <div className="w-full h-2 bg-primary-100 dark:bg-primary-900/50 rounded-full overflow-hidden">
                        <div className="h-full bg-secondary-500" style={{ width: `${opportunity.match}%` }}></div>
                      </div>
                    ...
                    <div className="flex justify-between">
                      <div className="flex items-center">
                        <div className="w-10 h-10 rounded-md overflow-hidden bg-primary-100 dark:bg-primary-900 flex items-center justify-center mr-3">
                          {opportunity.logoUrl ? (
                            <img src={opportunity.logoUrl} alt={opportunity.company} className="w-full h-full object-cover" />
                          ) : (
                      ...
                      <div className="px-2 py-1 bg-primary-100 dark:bg-primary-900/50 text-primary-700 dark:text-primary-300 rounded text-xs h-fit">
                        {opportunity.type === 'job' ? 'Job' : 'Micro-internship'}
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent>
