...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #f5f5f0;">
  <div style="max-width: 600px; margin: 0 auto; background-color: white; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
    ...
    // For testing: only send to verified email, otherwise log
    const verifiedEmail = 'gisachrismunyangaju@gmail.com'
    const targetEmail = email.toLowerCase() === verifiedEmail.toLowerCase() ? email : verifiedEmail

    console.log(`Original recipient: ${email}`)
    if (targetEmail !== email) {
      console.log(`Redirecting to verified email: ${targetEmail} (Resend free tier limitation)`)
    }
    ...
    const message = targetEmail !== email 
      ? `Email sent to ${targetEmail} (redirected from ${email} due to Resend free tier)`
